...
$(document).ready(function () {
    ...
    var svg = d3.select("#shortList-content").append("svg").attr("width", width + margin.left + margin.right).attr("height", height + margin.top + margin.bottom).append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")");

    svg.append("svg:clipPath").attr("id", "centerAvatarClip").append("svg:circle").attr("cx", "0").attr("cy", "0").attr("r", 40);

    svg.append("svg:use").attr("id", "topmost").attr("xlink:href", "");

    var force = d3.layout.force().gravity(.03).distance(200).charge(-400).size([width, height]);
    ...
    var node = svg.selectAll(".node").data(nodes).enter().append("g").attr("class", "node").attr("id", function (d) {
        return d.id;
    }).call(force.drag);

    node.append("image").attr("xlink:href", function (d) {
        return d.img;
    }).attr("x", -40).attr("y", -40).attr("width", "80px").attr("height", "80px").attr("clip-path", "url(#centerAvatarClip)");

    node.append("svg:circle").attr("class", "aux-border").attr("cx", "0").attr("cy", "0").attr("r", 40);

    node.append("svg:circle").attr("class", function (d) {
        ...
        return border;
    }).attr("cx", "35").attr("cy", "-35").attr("r", 15);

    node.append("text").attr("dx", 35).attr("dy", -30).attr("text-anchor", "middle").attr("class", "score-label").text(function (d) {
        return d.score;
    });

    node.append("text").attr("dx", 0).attr("dy", 55).attr("text-anchor", "middle").text(function (d) {
        return d.name;
    });
    ...
    var showExpandedView = function showExpandedView(d3Element) {
        ...
        $.get(source, (function (result) {
            if (result != null) {
                ...
                var person_expanded = d3Element.append("svg:g").attr("id", "person-expanded").attr("class", "person-expanded person expanded");

                person_expanded.append("svg:rect").attr("class", "expanded-bg").attr("width", 420).attr("height", 250).attr("x", -60).attr("y", -60).attr("rx", 10).attr("ry", 10);

                person_expanded.append("svg:g").attr("clip-path", "url(#centerAvatarClip)")
                //   .attr("transform", "translate(8,8)")
                .append("svg:image").attr("xlink:href", function (d) {
                    return d.img || missing_image;
                }).attr("width", "80px").attr("height", "80px").attr("x", -40).attr("y", -40);

                person_expanded.append("svg:circle").attr("class", "aux-border").attr("cx", "0").attr("cy", "0").attr("r", 40);

                person_expanded.append("svg:circle").attr("class", function (d) {
                    var border = "score-green";
                    if (d.score < 2.5) border = "score-red";
                    if (d.score >= 2.5 && d.score < 5) border = "score-yellow";
                    if (d.score >= 5 && d.score < 7.5) border = "score-blue";
                ...
                person_expanded.append("text").attr("dx", 35).attr("dy", -30).attr("text-anchor", "middle").attr("class", "score-label").text(function (d) {
                    return d.score;
                });

                person_expanded.append('foreignObject').attr('x', 45).attr('y', -20).attr('width', 300).attr('height', 150).append("xhtml:div").attr("id", "profileDetails").html(html);
                ...
                var closeBtn = person_expanded.append("svg:foreignObject").attr("width", 25).attr("height", 25).attr("y", "-45px").attr("x", "325px");

                closeBtn.append("xhtml:span").attr("class", "control glyphicon glyphicon-remove").style("cursor", "pointer").on("click", function (d, i) {
    ...
    var showPictureOnlyView = function showPictureOnlyView(d3Element) {
        // Remove the person-expanded view
        //d3.selectAll(d3Element[0][0].childNodes).remove();
        d3Element.selectAll("*").remove();

        // Add element again...... picture / score / name
        d3Element.append("image").attr("xlink:href", function (d) {
            return d.img;
        }).attr("x", -40).attr("y", -40).attr("width", "80px").attr("height", "80px").attr("clip-path", "url(#centerAvatarClip)");

        d3Element.append("svg:circle").attr("class", "aux-border").attr("cx", "0").attr("cy", "0").attr("r", 40);

        d3Element.append("svg:circle").attr("class", function (d) {
            var border = "score-green";
            if (d.score < 2.5) border = "score-red";
            if (d.score >= 2.5 && d.score < 5) border = "score-yellow";
            if (d.score >= 5 && d.score < 7.5) border = "score-blue";
        ...
        d3Element.append("text").attr("dx", 35).attr("dy", -30).attr("text-anchor", "middle").attr("class", "score-label").text(function (d) {
            return d.score;
        });

        d3Element.append("text").attr("dx", 0).attr("dy", 55).attr("text-anchor", "middle").text(function (d) {
            return d.name;
        });
