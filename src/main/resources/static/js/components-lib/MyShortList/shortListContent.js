...
var ShortListContent = React.createClass({
    ...
    componentWillReceiveProps: function componentWillReceiveProps(nextProps) {
        if (!$.isEmptyObject(nextProps.shortlist) && !$.isEmptyObject(nextProps.user) && $('#shortList-tree').html().length == 0) {
            ...
            var svg = d3.select("#shortList-tree").append("svg").attr("width", width + margin.left + margin.right).attr("height", height + margin.top + margin.bottom).append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")");
            ...
            node.on("mouseover", function (d) {
                ...
                console.log(d.id);
                var myModal = React.createElement(MyModal, { id: d.id, username: d.avatar, inititals: d.initials, handleHideModal: this.handleHideModal });
    ...
    render: function render() {
        return React.createElement(
            'div',
            { className: 'tab-pane fade text-left', id: 'shortList-content' },
            React.createElement('div', { id: 'shortList-tree' }),
            React.createElement('div', { ref: 'my-modal-2', id: 'my-modal-2' })
        );
    }
});