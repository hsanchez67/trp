...
var Result2 = React.createClass({
    ...
    render:function() {
        let displayNotLatchCheck = classNames({
            'pull-right hidden': this.props.status != 'Dejected',
            'pull-right show': this.props.status == 'Dejected'
        });
        return (
            <div className={this.props.count%2===0 ? 'row resultInstant' : 'row resultInstantGrey'}>
...
var DeletedResults = React.createClass({
    getInitialState: function () {
        return {
            loading: true
        };
...
var DeletedContent = React.createClass({
    displayName: 'DeletedContent',
    getInitialState: function(){
        return {
            communicationsTo: [],
            count: 0
    ...
    componentWillReceiveProps: function(nextProps) {
        console.log("DeletedContent:componentWillReceiveProps");
        console.log(nextProps);
        if (nextProps.count != this.state.count) {
            console.log("DeletedContent:componentWillReceiveProps:updateInbox");
            var id = $('#id').val();
            this.getCommunicationsTo(id);
        }
    },
    getCommunicationsTo: function(id) {
        ...
        $.ajax({
            type: "POST",
            url: "/getDeletedCommunicationsTo",
                                    ...
                                    <header className="panel-title">
                                        <div className="row">
                                            <div className="col-md-8 col-md-offset-2 text-center">
                                                <i className="fa fa-trash"></i>&nbsp;<strong>Trash</strong>
                                            </div>
                                            <div className="col-md-2">
                                                <a className="video" title="Managing Messages - Trash Folder" href="https://youtu.be/lChboJPtrNM"><img className="video-icon pull-right" src="/images/youTube.png" alt="Latch channel" /></a>
                                            </div>
                                        </div>
                                    </header>
                                </div>
                                <div className="panel-body">
                                    <DeletedResults data={this.state.communicationsTo} />
                                </div>
                            </div>
                        </div>
                    </div>
